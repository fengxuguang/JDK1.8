...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
     ...
     */
    /**
     * 用来控制表初始化和扩容的，默认值为 0，当在初始化的时候指定了大小，这会将这个大小保存在 sizeCtl中
     * 大小为数组的 0.75，当为负的时候，说明表正在初始化或扩张
     * -1 表示初始化
     * -(1 + n) n: 表示活动的扩张线程
     */
    ...
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        // key 值不能为 null, value 值也不能为 null, HashMap 中 key 和 value 值可以均为 null, (区别)
     ...
     */
    /**
     * TreeBin 用作树的头节点，只存储 root 和 first 节点，不存储节点的 key、value 的值
     * @param <K>
     * @param <V>
     */
